...
class IDPartner {
  ...
  async getAuthorizationUrl(query, proofs, scope) {
    ...
    const { iss: issuer, visitor_id: visitorId } = query;
    if (!issuer) {
      return `${oidcProxyServiceUrl}/auth/select-accounts?client_id=${clientId}&visitor_id=${visitorId}`;
    }
    ...
    // console.log(`pushedAuthRequest: `, pushedAuthRequest);
    // console.log(`searchParams: `,  searchParams.toString());
    // console.log(`issuer: `, client.issuer.authorization_endpoint);
    // console.log(`issuer url: `, `${client.issuer.authorization_endpoint}?${searchParams}`);
    return `${client.issuer.authorization_endpoint}?${queryParams}`
  }

  async claims(codeResponse, expectedIssuer, { state, nonce, codeVerifier }) {
    ...
    const tokenSet = await client.callback(client.redirect_uris[0], params, { jarm: true, state, nonce, code_verifier: codeVerifier });
