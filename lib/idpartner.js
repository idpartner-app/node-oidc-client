...
class IDPartner {
  ...
  async bankAccountsInfo(expectedIssuer, accessToken, options={}) {
    const client = await this.#getClient(expectedIssuer);

    const httpClient = createHttpClient(client.issuer.bank_accounts_info_endpoint);
    ...
    // console.log('client.issuer.bank_accounts_info_endpoint', client.issuer.bank_accounts_info_endpoint);
    // console.log('client.metadata', client.metadata);

    // return client.requestResource(client.metadata.bank_accounts_endpoint, accessToken, options);
    // return client.requestResource(new url.URL(client.issuer.bank_accounts_info_endpoint), accessToken, options);
  }
