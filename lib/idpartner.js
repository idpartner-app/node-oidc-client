...
class IDPartner {
  ...
  async paymentProcessing(expectedIssuer, accessToken, options = { method: 'POST', body: JSON.stringify({ amount: 0 }) }) {
    const client = await this.#getClient(expectedIssuer);
    const mTLS = !!client.tls_client_certificate_bound_access_tokens;
    ...
    const response = await client.requestResource(targetURL, accessToken, { ...options, method: 'POST' });
    const status = response.statusCode;
    if (status >= 400 && status < 600) {
